...
// =============================================================================
//  BASIC GPU VARIANT  (small N; Newton's 3rd law + SoA, one thread per particle)
// =============================================================================

// SoA device buffers
static double *ngd_x = nullptr, *ngd_y = nullptr, *ngd_z = nullptr;
static double *ngd_vx = nullptr, *ngd_vy = nullptr, *ngd_vz = nullptr;
static double *ngd_fx = nullptr, *ngd_fy = nullptr, *ngd_fz = nullptr;
static double *ngd_pe_arr = nullptr, *ngd_pe_total = nullptr;
static unsigned int ngd_n = 0;
static double NG_V_SHIFT = 0.0;

// Each thread owns particle i and computes pairs (i, j>i).
// Forces on i accumulate locally; forces on j use atomicAdd (sm_70 native double atomics).
// pe_arr[i] = PE contribution from all pairs (i, j>i) — summed to get total PE.
__global__ void ngk_forces(
    const double * __restrict__ x, const double * __restrict__ y, const double * __restrict__ z,
    double * __restrict__ fx,      double * __restrict__ fy,      double * __restrict__ fz,
    ...
    unsigned int n, double box_size, double v_shift)
{
    ...
    for (unsigned int j = i + 1; j < n; j++) {
        ...
        double sr12 = sr6 * sr6;
        double fij  = 24.0 * EPSILON * (2.0 * sr12 - sr6) * inv_r2;
        double fjx  = fij * dx, fjy = fij * dy, fjz = fij * dz;
        Fx += fjx; Fy += fjy; Fz += fjz;           // accumulate locally for i
        atomicAdd(&fx[j], -fjx);                    // Newton's 3rd law for j
        atomicAdd(&fy[j], -fjy);
        atomicAdd(&fz[j], -fjz);
        Pe += 4.0 * EPSILON * (sr12 - sr6) - v_shift; // each pair counted once — no 0.5
    }
    atomicAdd(&fx[i], Fx);
    atomicAdd(&fy[i], Fy);
    atomicAdd(&fz[i], Fz);
...
__global__ void ngk_kick_drift(
    double * __restrict__ x,  double * __restrict__ y,  double * __restrict__ z,
    double * __restrict__ vx, double * __restrict__ vy, double * __restrict__ vz,
    const double * __restrict__ fx, const double * __restrict__ fy, const double * __restrict__ fz,
    unsigned int n, double box_size)
...
static void ng_alloc(unsigned int n) {
    if (ngd_n == n) return;
    const size_t sz = (size_t)n * sizeof(double);
    CUDA_CHECK(cudaMalloc(&ngd_x,  sz)); CUDA_CHECK(cudaMalloc(&ngd_y,  sz)); CUDA_CHECK(cudaMalloc(&ngd_z,  sz));
    CUDA_CHECK(cudaMalloc(&ngd_vx, sz)); CUDA_CHECK(cudaMalloc(&ngd_vy, sz)); CUDA_CHECK(cudaMalloc(&ngd_vz, sz));
    CUDA_CHECK(cudaMalloc(&ngd_fx, sz)); CUDA_CHECK(cudaMalloc(&ngd_fy, sz)); CUDA_CHECK(cudaMalloc(&ngd_fz, sz));
    CUDA_CHECK(cudaMalloc(&ngd_pe_arr,   sz));
    CUDA_CHECK(cudaMalloc(&ngd_pe_total, sizeof(double)));
    ngd_n = n;
}

static void ng_upload(const Particle *p, unsigned int n) {
    const size_t sz = (size_t)n * sizeof(double);
    double *hx  = (double*)malloc(sz), *hy  = (double*)malloc(sz), *hz  = (double*)malloc(sz);
    double *hvx = (double*)malloc(sz), *hvy = (double*)malloc(sz), *hvz = (double*)malloc(sz);
    for (unsigned int i = 0; i < n; ++i) {
    ...
    CUDA_CHECK(cudaMemcpy(ngd_x,  hx,  sz, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(ngd_y,  hy,  sz, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(ngd_z,  hz,  sz, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(ngd_vx, hvx, sz, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(ngd_vy, hvy, sz, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(ngd_vz, hvz, sz, cudaMemcpyHostToDevice));
    free(hx); free(hy); free(hz); free(hvx); free(hvy); free(hvz);
}

static void ng_download(Particle *p, unsigned int n) {
    const size_t sz = (size_t)n * sizeof(double);
    double *hx  = (double*)malloc(sz), *hy  = (double*)malloc(sz), *hz  = (double*)malloc(sz);
    double *hvx = (double*)malloc(sz), *hvy = (double*)malloc(sz), *hvz = (double*)malloc(sz);
    double *hfx = (double*)malloc(sz), *hfy = (double*)malloc(sz), *hfz = (double*)malloc(sz);
    CUDA_CHECK(cudaMemcpy(hx,  ngd_x,  sz, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hy,  ngd_y,  sz, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hz,  ngd_z,  sz, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hvx, ngd_vx, sz, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hvy, ngd_vy, sz, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hvz, ngd_vz, sz, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hfx, ngd_fx, sz, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hfy, ngd_fy, sz, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(hfz, ngd_fz, sz, cudaMemcpyDeviceToHost));
    for (unsigned int i = 0; i < n; ++i) {
        p[i].x = hx[i];  p[i].y = hy[i];  p[i].z = hz[i];
        p[i].vx = hvx[i]; p[i].vy = hvy[i]; p[i].vz = hvz[i];
        p[i].fx = hfx[i]; p[i].fy = hfy[i]; p[i].fz = hfz[i];
...
static double ng_forces(unsigned int n, double box_size, int want_pe) {
    const unsigned int threads = 256;
    const unsigned int blocks  = (n + threads - 1) / threads;
    CUDA_CHECK(cudaMemset(ngd_fx, 0, (size_t)n * sizeof(double)));
    CUDA_CHECK(cudaMemset(ngd_fy, 0, (size_t)n * sizeof(double)));
    CUDA_CHECK(cudaMemset(ngd_fz, 0, (size_t)n * sizeof(double)));
    ngk_forces<<<blocks, threads>>>(
        ngd_x, ngd_y, ngd_z, ngd_fx, ngd_fy, ngd_fz, ngd_pe_arr,
        n, box_size, NG_V_SHIFT);
    if (!want_pe) return 0.0;
    CUDA_CHECK(cudaMemset(ngd_pe_total, 0, sizeof(double)));
    const unsigned int rth = 256;
    ngk_reduce_pe<<<(n + rth - 1) / rth, rth, rth * sizeof(double)>>>(
        ngd_pe_arr, ngd_pe_total, n);
    double pe = 0.0;
    CUDA_CHECK(cudaMemcpy(&pe, ngd_pe_total, sizeof(double), cudaMemcpyDeviceToHost));
    return pe;
}

SimulationResult run_simulation_gpu_basic_3d(Particle *particles, unsigned int n,
                                             unsigned int nsteps, double box_size,
                                             int log_steps) {
    ng_alloc(n);
    ng_upload(particles, n);
    NG_V_SHIFT = compute_v_shift();

    SimulationResult out;
    out.start_potential = ng_forces(n, box_size, 1);
    out.start_kinetic   = compute_ke(particles, n);
    out.start_total     = out.start_kinetic + out.start_potential;
    out.final_potential = out.start_potential;
    ...
    for (unsigned int step = 0; step < nsteps; ++step) {
        const int want_pe = log_steps || (step + 1u == nsteps);
        ngk_kick_drift<<<blocks, threads>>>(
            ngd_x, ngd_y, ngd_z, ngd_vx, ngd_vy, ngd_vz,
            ngd_fx, ngd_fy, ngd_fz, n, box_size);
        double pe = ng_forces(n, box_size, want_pe);
        ngk_kick<<<blocks, threads>>>(ngd_vx, ngd_vy, ngd_vz, ngd_fx, ngd_fy, ngd_fz, n);
        if (want_pe) out.final_potential = pe;
        if (log_steps) {
            CUDA_CHECK(cudaDeviceSynchronize());
            ng_download(particles, n);
            out.final_kinetic = compute_ke(particles, n);
            out.final_total   = out.final_kinetic + out.final_potential;
            printf("step=%6u KE=%12.6f PE=%12.6f E=%12.6f\n",
                   step, out.final_kinetic, out.final_potential, out.final_total);
    ...
    // printf("[BASIC] Newton's 3rd law + SoA  N=%u\n", n);
    // printf("Simulation time %u steps: %.3f seconds\n", nsteps, ms / 1000.0f);
    return out;
}
...
#define BASIC_N_THRESHOLD 300u
#define AUTO_N_THRESHOLD  43000u

// Route by size: basic Newton GPU for tiny N, graph for medium, warpnl for large.
SimulationResult run_simulation(Particle *particles, unsigned int n,
                                unsigned int nsteps, double box_size,
                                int log_steps) {
    if (n < BASIC_N_THRESHOLD) {
        fprintf(stdout, "[AUTO] N=%u < %u  ->  basic Newton GPU variant\n", n, BASIC_N_THRESHOLD);
        return run_simulation_gpu_basic_3d(particles, n, nsteps, box_size, log_steps);
    } else if (n <= AUTO_N_THRESHOLD) {
